...
namespace Blackboard.Core.Nodes.Inner {
    ...
    sealed public class BitwiseXor<T>: NaryValue<T, T>
        ...
        static public readonly IFuncDef Factory =
            new FunctionN<IValueParent<T>, BitwiseXor<T>>((values) => new BitwiseXor<T>(values));

        /// <summary>Creates a bitwise Exclusive OR value node.</summary>
        /// <param name="parents">The initial set of parents to use.</param>
        public BitwiseXor(params IValueParent<T>[] parents) :
            base(parents) { }

        /// <summary>Creates a bitwise Exclusive OR value node.</summary>
        /// <param name="parents">The initial set of parents to use.</param>
        /// <param name="value">The default value for this node.</param>
        public BitwiseXor(IEnumerable<IValueParent<T>> parents = null, T value = default) :
            base(parents, value) { }
